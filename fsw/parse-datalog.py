import struct
import sys

CRC_TABLE = [
    
]

def crc8(buf):
    checksum = 0
    for b in buf:
        checksum = CRC_TABLE[checksum ^ b]
    return checksum

FMT_STR = '<fhhhhhhffffH'
FRAME_LEN = struct.calcsize(FMT_STR)

GYRO_SCALE = 17.5 * 0.017453293 / 1000.0
ACC_SCALE = 0.061 * 9.80665 / 1000.0

def main(args):
    fp = open(args[1], 'rb')
    out_fp = open('datalog.csv', 'w')

    buf = fp.read(FRAME_LEN+1)
    checksum_failed = False
    while len(buf) == FRAME_LEN+1:
        # does our checksum match?
        checksum = buf[-1]
        expected_crc = crc8(buf[:-1])
        if expected_crc != checksum:
            if not checksum_failed:
                print('checksum failed')
            checksum_failed = True

            # shift the buffer and try again
            buf = buf[1:]+fp.read(1)
            continue
        
        if checksum_failed:
            print('checksum passed, resume extraction...')
        checksum_failed = False
        frame = struct.unpack(FMT_STR, buf[:-1])

        # scale & convert sensor data
        scaled_frame = []
        scaled_frame += [float(frame[0])]                           # timestamp
        scaled_frame += [GYRO_SCALE*float(f) for f in frame[1:4]]   # raw gyro
        scaled_frame += [ACC_SCALE*float(f) for f in frame[4:7]]    # raw accel
        scaled_frame += list(frame[7:11])                           # nav state
        scaled_frame += [frame[11]]

        print(','.join(map(str, scaled_frame)), file=out_fp)
        
        buf = fp.read(FRAME_LEN+1)

    print('done extracting data')
    fp.close()
    out_fp.close()


if __name__ == '__main__':
    exit(main(sys.argv))
